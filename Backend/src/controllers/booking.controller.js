...
const createBooking = async (req, res) => {
	...
	const [findError, apartment] = await to(
		Apartment.findOne({ "rooms._id": new mongoose.Types.ObjectId(roomId) }),
	);
	...
	const [saveError] = await to(newBooking.save());
	if (saveError) {
		return res
			.status(500)
			.json({
	...
	const [readError, htmlTemplate] = await to(
		fs.readFile("src/template/bookingConfirmationTemplate.html", "utf-8"),
	);
	if (readError) {
		return res
			.status(500)
	...
	const [mailError] = await to(
		sendMail({ email, html: htmlToSend, subject: "Booking Confirmation" }),
	);
	if (mailError) {
		return res
			.status(500)
