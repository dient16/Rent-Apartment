...
const register = async (req, res) => {
  try {
    ...
    if (err) {
      return res
        .status(500)
        .json({ success: false, message: "Error checking existing user" });
    }
    ...
    [err, newUser] = await to(
      User.create({ email, confirmationToken: confirmationToken }),
    );
    if (err) {
      return res
        .status(500)
    ...
    const [readError, htmlTemplate] = await to(
      fs.readFile("src/template/confirmMailTemplate.html", "utf-8"),
    );
    if (readError) {
    ...
    const [mailError] = await to(
      sendMail({ email, html: emailHtml, subject: "Confirm email" }),
    );
    if (mailError) {
      return res.status(500).json({
        success: false,
...
const confirmEmail = async (req, res) => {
  ...
  res.redirect(`${process.env.CLIENT_URL}/set-password/${user._id}`);
};
...
const setPassword = async (req, res, next) => {
  try {
    ...
    if (!user.emailConfirmed) {
      return res
        .status(400)
        .json({ success: false, message: "Email has not been confirmed" });
    }
...
const login = async (req, res, next) => {
  try {
    ...
    const [errPassword, isPasswordCorrect] = await to(
...
const refreshAccessToken = async (req, res, next) => {
  try {
    ...
    const [errFindUser, user] = await to(
      User.findOne({ _id: decodedToken._id, refreshToken }),
    );
